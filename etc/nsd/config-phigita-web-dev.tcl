proc config_phigita_web {configVar} {
    ...
    set directoryfile  $config(directoryfile)
    set minthreads     $config(minthreads)
    set maxthreads     $config(maxthreads)

    set performance_mode_p $config(performance_mode_p)
    set production_mode_p  $config(production_mode_p)
    set is_mail_server_p   $config(is_mail_server_p)
    set storage_port       $config(storage_port)
    set listening_to_host  $config(listening_to_host)


    set email          $config(email)
    set pagedir        $config(pagedir)

    set db_pool_connections $config(db_pool_connections)
    set debug          $config(debug)
    set datasource     $config(datasource)
    set password       $config(password)
    set user           $config(user)


    set homedir        $config(homedir)
    ...
    ns_param listening_to_host ${listening_to_host}
...
ns_param   datasource     "port=$db_port host=$db_host dbname=${db_name}"
...
ns_param   checkinterval 600
    ...
    ns_section "ns/server/${server}/limits"
