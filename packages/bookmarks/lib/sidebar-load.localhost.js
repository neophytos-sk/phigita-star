...
var offY = 15;
	...
	var baseUrl = "http://localhost:8090/my";
...
*/
	...
	*/
	var src = baseUrl + "/bookmarks/sidebar?" + pars.join("&");
	...
	iframeEl.src = baseUrl + "/bookmarks/sidebar-attach?t=image&u="+encodeURIComponent(imageUrl);
	iframeEl.width = 0;
	iframeEl.height = 0;
...
_reMark.stopCapture = function() {
	...
	xo.log('stopCapture:caught exception');
	xo.log(ex);
    }
};
...
function failsafe(err) {
    ...
    var loc = "http://localhost:8090/bookmarks/url-add?url=" +encodeURIComponent(sf_url)+ "&description=" +encodeURIComponent(sf_description);
    if (err != null) {
	loc += "&exception=" + err;
    }
