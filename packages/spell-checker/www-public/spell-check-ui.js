...
function makeCleanDoc(leaveFixed) {
	...
	return frame.contentWindow.document.body.innerHTML.replace(/^<pre( [^>]*)?>/gi, "").replace(/<\/pre>\s*$/gi, "");
};
...
function saveClicked() {
	if (modified) {
		window.opener.document.getElementById(SpellChecker._textarea).value = makeCleanDoc(false);
	}
	window.close();
	return false;
};
...
function initDocument() {
	...
	var field = document.getElementById("f_content");
	var f_dictionary = document.getElementById("f_dictionary");
	field.value = window.opener.document.getElementById(SpellChecker._textarea).value;
	f_dictionary.value = window.opener.document.getElementById(SpellChecker._dictionary).value;
	field.form.submit();
