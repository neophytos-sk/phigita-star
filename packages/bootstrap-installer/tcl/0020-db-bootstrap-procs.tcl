...
proc db_bootstrap_set_db_type { errors } {
    ...
    #set database_problem "PWS requires three database pools in order to run correctly."
    #}
    ...
    foreach pool $pools {
        if { [catch { set db [ns_db gethandle -timeout 15 $pool]}] || ![string compare $db ""] } {
            ...
            lappend bad_pools "<li>PWS could not allocate a handle from database pool \"$pool\"."
            set long_error 1
            break
        } else {
            set this_suffix ""
