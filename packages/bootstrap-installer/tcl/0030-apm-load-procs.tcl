ad_library {
    ...
    @cvs-id $Id: 0030-apm-load-procs.tcl,v 1.1.1.1 2002/11/22 09:47:31 nkd Exp $
}
...
ad_proc apm_guess_file_type { package_key path } {
    ...
    <li>Files <code>pkgIndex.info</code> are considered package specification files.
    ...
    set dirs_in_pageroot [llength [split [ns_pagedir] "/"]]	   ;# See comments by RBM
	...
	} elseif { [regexp -- "^(create|drop)\.sql" [file tail $path] "" kind] } {
	...
	set type "sqlj_code"
    } elseif { [lsearch $components "pkgIndex.info"] >= 0 } {
	set type "package_spec"
    } elseif { [string equal $extension ".xql"] } {
	set type "query_file"
    } elseif { [string equal $extension ".pdl"] } {
	set type "pdl_model"
...
ad_proc -private apm_guess_db_type { package_key path } {
    ...
    if { [string match "data_model*" [apm_guess_file_type $package_key $path]] } {
        ...
        if { $sql_index >= 0 && $package_key != "core-platform" && $package_key != "package-manager" } {
...
ad_proc apm_bootstrap_load_pdl {
    package_key
} {
    @author Neophytos Demetriou
} {
    set base "[acs_root_dir]/packages/$package_key/"
    set base_len [string length $base]
    set dir ${base}pdl
    set paths [list]
    
    set paths [concat $paths [glob -nocomplain "$dir/*.pdl"]]
    
    set files ""
    foreach path [lsort $paths] {
	set rel_path [string range $path $base_len end]
	lappend files [list $package_key $rel_path]
    }

    foreach file $files {
	lassign $file package filename
	source [acs_root_dir]/packages/${package}/$filename
    }
}
...
proc apm_bootstrap_load_queries { package_key } {
    ...
    # DRB: We can't parse the pkgIndex.info file at this point in time, primarily because
