...
proc ::isbn::convert_to_ean13 {isbn} {
    set isbn_no_dashes [string map {- {}} [string trim $isbn]]
    if { [string length ${isbn_no_dashes}] eq {13} && [string range ${isbn_no_dashes} 0 2] eq {978} } {
	set first12digits "978[string range ${isbn_no_dashes} 3 end-1]"	
	return "${first12digits}[ean13_csum ${first12digits}]"
    }
    if { [string length ${isbn_no_dashes}] eq {10} && [isbn::valid_checksum_p ${isbn}] } {
	set first12digits "978[string range ${isbn_no_dashes} 0 end-1]"
	return "${first12digits}[ean13_csum ${first12digits}]"
    }
    return {}
}
...
proc biblionet.convert.to.csv {line} {
    ...
    set isbn10 [string range $isbn10 5 end]
    set ean13 [::isbn::convert_to_ean13 $isbn10]
    if { [info exists exists_ean13($ean13)] } {
	return ""
    }
    set exists_ean13($ean13) ""
