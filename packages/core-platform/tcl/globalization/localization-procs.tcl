...
ad_library {
    ...
    @cvs-id $Id: localization-procs.tcl,v 1.5 2002/11/18 17:58:35 lars Exp $
}
...
ad_proc -public lc_time_fmt {
    ...
    #ns_log notice datetime=$datetime
    if { $tz eq {} } {
	set tz Europe/Athens
	catch { set tz [ClockMgr getLocalTZ] }
    }
    #ns_log notice tz=$tz
    lassign [split $datetime ".+"] datetime __microseconds_and_tz__
    set input_fmt "%Y-%m-%d %H:%M:%S"
    set datetime [clock format [clock scan $datetime -timezone :UTC]  -format $input_fmt -timezone :${tz}]

    #ns_log notice "tz=$tz datetime=$datetime"
...
set locale en_US
    ...
    set percent_match(l) [lc_time_drop_meridian $lc_time_hours]
