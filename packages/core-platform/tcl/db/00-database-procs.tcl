...
Places a usable database handle in $db and executes $code_block.
    ...
    ::xo::kit::log db=$db code_block=$code_block
    ...
    if { $db_state(n_handles_used) == 0 } {
	set my_dbh [::xo::db::gethandle]
    } else {
	if { $db_state(n_handles_used) >= [llength $db_state(handles)] } {
		...
		#HERE
		ns_db dml ${db} "set time zone 'Europe/Athens';"
	    } error]
...
proc_doc db_foreach { statement_name sql args } {
	...
	while { [db_getrow $db $selection] } {
	    ...
	    # due to the ::xo::db::withhandle wrapped around this loop, so propagate it explicitly.
	    switch $errno {
		0 {
		    # TCL_OK
		}
...
Columns in this query: [join [lsort -ascii $local_columns] ", "]" "" "ACS_MULTIROW_APPEND_COLUMNS_MISMATCH"
		...
		# "return -code $errno..." trick due to the ::xo::db::withhandle
...
ad_proc db_0or1row { statement_name sql args } { 
    ...
    ::xo::kit::log db_0or1row statement_name=$statement_name
...
ad_proc db_transaction { transaction_code args } {
    ...
    ::xo::db::withhandle db {
	# Preserve the handle, since ::xo::db::withhandle kills it after executing
	# this block.
	set dbh $db	
