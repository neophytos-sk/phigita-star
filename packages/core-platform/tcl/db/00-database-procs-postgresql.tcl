ad_library {
    ...
    @cvs-id $Id: 00-database-procs-postgresql.tcl,v 1.1.1.1 2002/11/22 09:47:31 nkd Exp $
}

# Returns the next value for a sequence. This can utilize a pool of sequence values to save hits to the database.
#::xo::db::value "select nextval('t_${sequence}')"
proc db_nextval { sequence } {
    return [::xo::db::nextval ${sequence}]
}
...
ad_proc -public db_table_exists { table_name } {
    ...
    set quoted_table_name '${table_name}'  ;# DANGEROUS, ns_dbquotevalue was raising an error
    set exists_p [::xo::db::value -statement_name table_exists_p -default 0 "select 1 from pg_class where relname = $quoted_table_name and relkind = 'r' limit 1"]
    return $exists_p
}
...
ad_proc -public db_column_exists { table_name column_name } {
    ...
    set columns [list]
    set exists_p [::xo::db::value -statement_name column_exists -default 0 "select 1 from user_tab_columns where table_name = upper([ns_dbquotevalue $table_name]) and column_name = upper([ns_dbquotevalue $column_name]) limit 1"]
    return $exists_p
}
...
ad_proc -public db_column_type { table_name column_name } {
    ...
    return [::xo::db::value -statement_name column_type_select -default "-1" "select data_type as data_type from user_tab_columns where upper(table_name) = upper([ns_dbquotevalue $table_name]) and upper(column_name) = upper([ns_dbquotevalue $column_name])"]
