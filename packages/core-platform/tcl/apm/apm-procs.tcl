ad_library {
    ...
    @cvs-id $Id: apm-procs.tcl,v 1.1.1.1 2002/11/22 09:47:32 nkd Exp $
}
...
ad_proc -private apm_load_pdl_files { 
} {
	...
	lappend spec_files [acs_root_dir]/packages/$pkg/pkgIndex.info
    }

    set packages [my_dependency_check $packages]
    # Scan the package directory for files to source.    
    set files [list]    
    foreach package $packages {
	...
	set path [acs_root_dir]/packages/$package
	if { ![file exists $path] } { 
	    ns_log notice "missing path=$path"
	    continue
	}
	if { [catch {
	    ns_log notice "Sourcing pdl file: $file"
...
ad_proc -private apm_files_load {    
    ...
    foreach file_info $files {
        util_lassign $file_info package_key path
