ad_library {
    ...
    @cvs-id $Id: apm-xml-procs.tcl,v 1.1.1.1 2002/11/22 09:47:32 nkd Exp $
} 
...
#  	foreach file [glob "[acs_package_root_dir acs-kernel]/tcl/xml-*-procs.tcl"] {
#  	    apm_source $file
#  	}
...
ad_proc -private apm_attribute_value {
    ...
    set value [$nodeObject getAttribute $attributeName $defaultValue]
...
ad_proc -public apm_read_package_info_file { path } {
    ...
    Reads a pkgIndex.info file, returning an array containing the following items:
    ...
    <p>If the pkgIndex.info file cannot be read or parsed, this routine throws a
    descriptive error.
    ...
    # If the pkgIndex.info file hasn't changed since last read (i.e., has the same
    ...
    set root_node [$tree documentElement] ;#xml_doc_get_first_node_by_name $tree package
    ns_log Debug "XML: root node is [xml_node_get_name $root_node]"
	...
	error "Package must contain exactly one <database-support> node"
        ...
        foreach database [xml_node_get_children_by_name [lindex $database_support_sections 0] "database"] {
            lappend properties(database_support) [xml_node_get_content $database]
        }
    }
