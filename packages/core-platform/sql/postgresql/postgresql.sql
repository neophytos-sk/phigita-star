...
begin
  ...
  if p_intkey < 128 then
    return substring("bit"(p_intkey), 25, 8);
  else
    return substring("bit"((-2^31)::int + p_intkey), 1, 32);
  end if;
...
begin
  ...
  if substring(p_tree_key, v_parent_pos, 1) = ''1'' then
    return "int4"(substring(p_tree_key, v_parent_pos + 1, 31));
  else
    return "int4"(substring(p_tree_key, v_parent_pos, 8));
  end if;
...
begin
  ...
  if substring(p_tree_key, v_leaf_pos, 1) = ''1'' then
    return "int4"(substring(p_tree_key, v_leaf_pos + 1, 31));
  else
    return "int4"(substring(p_tree_key, v_leaf_pos, 8));
  end if;
