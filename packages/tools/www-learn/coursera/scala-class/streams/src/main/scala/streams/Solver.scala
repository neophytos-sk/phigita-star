...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case (block,moves) #:: rest => {
      lazy val newNeighbors = newNeighborsOnly(neighborsWithHistory(block, moves),explored)
      initial.head #:: from(rest ++ newNeighbors, explored + block)
    }
  }
  ...
  lazy val pathsToGoal: Stream[(Block, List[Move])] = 
    pathsFromStart filter (_._1 == Block(goal,goal))
