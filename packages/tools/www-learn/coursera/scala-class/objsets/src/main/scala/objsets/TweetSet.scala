...
abstract class TweetSet {
  ...
  def ascendingByRetweet0(acc: Trending): Trending
...
class Empty extends TweetSet {
  ...
  def filter0(p: Tweet => Boolean, acc: TweetSet): TweetSet = acc
  ...
  def ascendingByRetweet0(acc: Trending): Trending = acc
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filter0(p: Tweet => Boolean, acc: TweetSet): TweetSet = 
    if (p(elem)) left.filter0(p, right.filter0(p, acc incl elem)) else left.filter0(p, right.filter0(p, acc))
  ...
  // -------------------------------------------------------------------------
  def union(that: TweetSet): TweetSet = 
    (tail union that) incl head
    
  def ascendingByRetweet0(acc: Trending): Trending = {
    val theMin = findMin
    remove(theMin).ascendingByRetweet0(acc + theMin)
  }
...
object GoogleVsApple {
  ...
  val googleTweets: TweetSet = TweetReader.allTweets.filter(tw => google.exists(w => tw.text.contains(w)))

  val appleTweets: TweetSet = TweetReader.allTweets.filter(tw => apple.exists(w => tw.text.contains(w)))
