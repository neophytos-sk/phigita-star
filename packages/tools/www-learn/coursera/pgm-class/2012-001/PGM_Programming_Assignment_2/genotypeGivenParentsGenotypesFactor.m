...
genotypeFactor.card = numGenotypes * ones(1,3);
...
for k=1:length(assignments)
  G = assignments(k,:);
  genotypeChild = G(1);
  genotypeParentOne = G(2);
  genotypeParentTwo = G(3);

  allelesParentOne = genotypesToAlleles(genotypeParentOne,:);
  allelesParentTwo = genotypesToAlleles(genotypeParentTwo,:);

  num_occurrences = 0;
  for i=1:length(allelesParentOne)
    for j=1:length(allelesParentTwo)
      if genotypeChild == allelesToGenotypes(allelesParentOne(i),allelesParentTwo(j))
	num_occurrences += 1;
      end
    end
  end

  genotypeFactor.val(k) =  num_occurrences / (length(allelesParentOne)*length(allelesParentTwo));
  % genotypeFactor.val(k) =  num_occurrences / 4;
