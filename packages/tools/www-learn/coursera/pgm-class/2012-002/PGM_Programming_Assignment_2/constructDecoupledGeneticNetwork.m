...
for i = 1:numPeople
  geneCopyVarOne = i;
  geneCopyVarTwo = numPeople+i;
  phenotypeVar = 2*numPeople+i;

  parents = pedigree.parents(i,:);
  if (parents(1) == 0) 
    factorList(geneCopyVarOne) = childCopyGivenFreqsFactor(alleleFreqs,geneCopyVarOne);
    factorList(geneCopyVarTwo) = childCopyGivenFreqsFactor(alleleFreqs,geneCopyVarTwo);
  else
    parentVarOne = parents(1);
    parentVarTwo = parents(2);
    factorList(geneCopyVarOne) = childCopyGivenParentalsFactor(numAlleles, geneCopyVarOne, parentVarOne, numPeople+parentVarOne);
    factorList(geneCopyVarTwo) = childCopyGivenParentalsFactor(numAlleles, geneCopyVarTwo, parentVarTwo, numPeople+parentVarTwo);
  end

  factorList(phenotypeVar) = phenotypeGivenCopiesFactor(alphaList, numAlleles, geneCopyVarOne, geneCopyVarTwo, phenotypeVar);
