ad_page_contract {
    ...
    @cvs-id $Id: packages-install-4.tcl,v 1.1.1.1 2002/11/22 09:47:32 nkd Exp $
} {
    {sql_file:multiple ""}
}
...
foreach index $sql_file {
    ...
    set file [lindex $sql_file_paths $index]
    lappend sql_files $file
    if { [string compare [lindex $file 1] pdl_model] == 0 } {
	lappend pdl_files $file
    }
}

ns_write "PDL FILES!!! $pdl_files"
