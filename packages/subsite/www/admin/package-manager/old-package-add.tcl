ad_page_contract {
    ...
    @date 17 April 2000
    @cvs-id $Id: old-package-add.tcl,v 1.1.1.1 2002/11/22 09:47:32 nkd Exp $
} {
}
...
db_foreach layer {
    ...
    append layer_options "<option value=${layer_id}>${name}</option>"
}
...
function updateURLs() {
    ...
    if (form.package_uri.value == '')
        form.package_uri.value = 'http://www.xoacs.org/repository/apm/packages/' + form.package_key.value;
    if ((form.version_name.value != '') && (form.version_uri.value == ''))
        form.version_uri.value = 'http://www.xoacs.org/repository/download/apm/' + form.package_key.value + '-' + form.version_name.value + '.apm';
}
</script>
...
<tr>
  ...
  Services are system-level software that extend Xoacs to provide new system-wide functionality,
  e.g. Workflow.
</tr>
<tr>
...
<tr>
  <th align=right nowrap>Layer:</th>
  <td><select name=layer_id>
      $layer_options
<tr>
  <th align=right nowrap>Xoacs Core?</th>
  <td><input type=checkbox name=initial_install_p value=t> Is your package part of the Xoacs Core that
forms the set of packages initially installed?  If you're not part of the Xoacs Core development team,
...
<tt>mailto:developername@xoacs.org</tt>).
</tr>
