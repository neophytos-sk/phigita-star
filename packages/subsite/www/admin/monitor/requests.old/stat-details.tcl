...
set total_avg [expr {$cnt>0 ? $total/($cnt*1000.0) : "na" }]
...
template::list::create \
	...
	orderby url
	  display_template {
	      <a href=@stat_details.url@>@stat_details.url@</a>
	  }
      }
      totaltime {
        label "Total Time"
...
foreach l [lsort $type $order -index $index $full_stat] {
  ...
  if {[string length $url]>100} {set url [string range $url 0 97]...}
  multirow append stat_details \
      $url [lindex $l 1] [lindex $l 2] \
