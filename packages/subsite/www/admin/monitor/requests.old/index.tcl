...
proc graph values {
  ...
  foreach v $values {
    append graph "<tr><td width=500><img title='$v' src='$pixel_file' width=[expr {int(450.0*$v/$max)}] height=2></td>\n</tr>"
  }
  append graph "</table>\n"
  return $graph
}
...
proc counterTable {label objlist} {
    append text "<div style=\"clear:both;\" /><table align=center>" \
      "<tr><td width=100></td><td width=500>Trend</td><td width=300>Max</td></tr>"
  foreach {t l} $objlist {
    set trend [throttleThread do $t set trend]
    append text \
        "<tr><td style='text-align: center; border: 1px solid blue;width:100;'>$label per <br>$l</td>" \
        "<td style='padding: 5px; border: 1px solid blue;width:510;'>[graph $trend]<font size=-2>$trend</font></td>" \
...
set running [throttle do array size running_url]
...
set active_user_string "$active_users_10 active users in last 10 minutes, $activeUsers24 in last $::server_running ($activeTotal24 total)"
...
if {[string compare "" [info command ad_return_template]]} {
 ...
 <HTML><TITLE>System Statistics</TITLE><BODY>
      <div style="clear:both;" /><table align=center style="border: 1px solid blue; padding: 10px;"
