ad_library {
    ...
    @cvs-id $Id: 00-tcl-documentation-procs.tcl,v 1.1.1.1 2002/11/22 09:47:33 nkd Exp $
}
...
ad_proc -public ad_page_contract {
	...
	if { [info exists apc_internal_filter($formal_name:stx)] } {
	    set actual_value [stx_from_input ${actual_value}]
	    ad_page_contract_set_validation_passed ${formal_name}:stx
	}
...
ad_page_contract_filter alnum { name value } {
    @author Neophytos Demetriou
} {
    if { ![string is alnum ${value}] } {
	ad_complain "Value for $name is not valid"
	return 0
    }
    return 1
}
...
ad_page_contract_filter label { name value } {
    @author Neophytos Demetriou
} {
    set value [string trim $value]
    return 1
    if { ![string is alnum [string map {- "" _ "" " " ""} $value]] } {
	ad_complain "Value for ${name} is not valid"
	return 0
    }
}

ad_page_contract_filter ltree { name value } {
    @author Neophytos Demetriou
} {
    set value [string trim $value]
    return 1
    foreach label [split [string map {- "" _ "" " " ""} $value] .] {
	if { ![string is alnum ${label}] } {
	    ad_complain "Value for $name is not valid"
	    return 0
	}
    }
...
ad_page_contract_filter boolean { name value } {
    ...
    if {[string match $value "0"] || \
	    ...
	    [string match $lcase_value "on"] || \
	    [string match $lcase_value "off"] || \
...
ad_page_contract_filter_rule html { name filters } {
	...
	if { [lsearch { stx nohtml html allhtml integer naturalnum } $flag] != -1 } {
	    return
	}
    }
