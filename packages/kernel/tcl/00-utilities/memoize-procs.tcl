...
ad_proc -public util_memoize {script {max_age ""}} {
	...
	return [ns_cache_eval -expires [expr {$current_time + $max_age}] util_memoize $script $script]
    } else {
	return [ns_cache_eval util_memoize $script $script]
    }
}



ad_proc -public util_memoizeold {script {max_age ""}} {
    If <i>script</i> has been executed before, return the value it
    returned last time, unless it was more than <i>max_age</i> seconds ago.
    ...
    set cached_p [ns_cache_eval util_memoize $script pair]
...
ad_proc -public util_memoize_seed {script value {max_age ""}} {
    ...
    ns_cache_eval util_memoize $script { return $value }
}
...
ad_proc -private util_memoize_flush_local {script} {
    ...
    ns_cache_flush util_memoize $script
}
...
ad_proc -public util_memoize_cached_p {script {max_age ""}} {
    ...
    if {![ns_cache_eval util_memoize $script {}]} {
        return 0
    }
...
ad_proc -public util_memoize_initialized_p {} {
    ...
    return [ad_decode [catch {ns_cache_eval util_memoize __util_memoize_installed_p {return 1}} error] 0 1 0]
}
...
ad_proc -public util_memoize_flush_regexp {
    ...
    foreach name [ns_cache_keys util_memoize] {
       if $log_p {
