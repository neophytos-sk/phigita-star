ad_library {
    ...
    @cvs-id $Id: navigation-procs.tcl,v 1.1.1.1 2002/11/22 09:47:33 nkd Exp $
...
proc ad_context_bar { args } {
    ::xo::html::iuse {compact}
    ...
    foreach label [split [string trim $package_url /] /] {
	append url ${label}/
	array set node [nsv_get site_nodes ${root_of_host}${url}]
	if {[string equal ${label} user-homepage]} {
	...
	if { $i == [llength $context] - 1 } {
	    if {[llength $args] == 0} {
		lappend out "<li>[lindex $element 1]</li>"
	    } else {
		lappend out "<li>$element</li>"
	    }
	} else {
	    lappend out "<li><a class=\"fl\" href=\"[lindex $element 0]\">[lindex $element 1]</a></li>"
	}
    }

    return "<ul class=\"compact\">[join $out " <li>&gt;</li> "]</ul>"
...
proc_doc ad_admin_context_bar args { 
    ...
    set choices [list "[ad_pvt_home_link]" "<a href=\"/control-panel/\">ACS System Wide Administration</a>"]
    set index 0
    foreach arg $args {
	incr index
	if { $index == [llength $args] } {
