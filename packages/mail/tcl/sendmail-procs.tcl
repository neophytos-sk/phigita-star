...
proc ::xo::mail::sendmail {to from subject body {headers {}} {bcc {}} {cc {}}} {
    ...
    set email [lindex $to 0] ;# HERE
    # TODO: regexp {.*<(.*)>} $to _dummy_ email
    lassign [split $email "@"] user domain

    if { [catch { set smtplist [lsort -index 0 [::xo::dns::resolve $domain MX]] } errMsg] } {
	ns_log error "errMsg=$errMsg"
	if { [string match "Name Error - domain does not exist" $errMsg] } {
	    return 0
	} else {
	    return -1
	}
    ...
    set timeout  [ns_config ns/parameters smtptimeout 30]

    ## if {$smtphost eq {}} { 
    ##    set smtphost [ns_config ns/parameters mailhost "localhost"]
    ## }

    # TODO: iterate over smtplist
    # ::xo::dns::resolve gmail.com MX
    # => {5 gmail-smtp-in.l.google.com} {20 alt2.gmail-smtp-in.l.google.com} {40 alt4.gmail-smtp-in.l.google.com} {30 alt3.gmail-smtp-in.l.google.com} {10 alt1.gmail-smtp-in.l.google.com}

    set smtphost [lindex [lindex $smtplist 0] 1]
    ns_log notice "sending mail directly to smtphost=$smtphost timeout=$timeout smtpport=$smtpport"
    ...
    set err [catch {
	    ...
	    ns_log notice [list _ns_smtp_send "Helo" $wfd "HELO $host"]
