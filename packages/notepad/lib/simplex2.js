...
Server.requestSessionInfo = function(a){ //Get the User Prefix
...
Simplex.textarea_swapRoles = function(nosave){ //This function swaps the textareas' roles.
    ...
    DH.removeClass(primaryEditorEl, xo.getCssName("hovered"));
    DH.removeClass(secondaryEditorEl, xo.getCssName("fixed"));
    DH.addClass(secondaryEditorEl, xo.getCssName("hovered"));
    DH.addClass(primaryEditorEl, xo.getCssName("fixed"));
    ...
    // hide textEl
    DH.removeClass(Simplex.textSecDiv, xo.getCssName("editing"));
    DH.addClass(Simplex.textPriDiv, xo.getCssName("editing"));
...
Simplex.textarea_moveTo = function(el){
    ...
    DH.addClass(editorEl,xo.getCssName("hovered"));
...
Simplex.edit = function(id,pos,newtext) {
    ...
    var projectEl = Simplex.projectDom(el);
    if (DH.hasClass(projectEl,xo.getCssName("parent"))) {
...
Simplex.textarea_edit = function(el, pos, newtext){ //Set the secondary area as EDITABLE (active) (swap roles and focus)
	...
	xo.log('textarea_edit-1'+Simplex.textPriDiv);
...
Simplex.handleMoveHandleMouseDown = function(e,target,options) {
    ...
    DH.addClass(projectEl,xo.getCssName("moving"));
...
Simplex.handleMoveHandleMouseUp = function(e,target,options) {
	...
	DH.removeClass(Simplex.__dragProjectEl,xo.getCssName("moving"));
	xo.log(e.target);
	xo.Event.stopEvent(e);
...
Simplex.textarea_save = function(noSyncPriDiv,objCurr){
    ...
    xo.log(ta);
    xo.log(Simplex.textSec);
    xo.log('ta_save content='+content);
    if (!noSyncPriDiv) {
...
Simplex.setEditorClasses = function(editorEl,projectEl,contentEl) {
    ...
    if(DH.hasClass(contentEl.parentNode,xo.getCssName("name"))) {
	DH.addClass(editorEl, xo.getCssName("nameEditor"));
    } else {
	DH.addClass(editorEl, xo.getCssName("noteEditor"));
    }

    if(DH.hasClass(projectEl,xo.getCssName("selected"))) {
	DH.addClass(editorEl, xo.getCssName("selectedEditor"));
    } else {
	// if (!DH.hasClass(projectEl,"parent")) {
	...
	if (objCurr[_parent_] && objCurr[_parent_] != Simplex.__zoomRoot) 
	    DH.addClass(editorEl,xo.getCssName("twoLevelsDownEditor"));
	else
	    DH.removeClass(editorEl,xo.getCssName("twoLevelsDownEditor"));
	//}
    }

    if (DH.hasClass(projectEl,xo.getCssName("done"))) {
	DH.addClass(editorEl, xo.getCssName("doneEditor"));
    }

}

Simplex.clearEditorClasses = function(editorEl) {
    DH.removeClass(editorEl,[xo.getCssName("nameEditor"), xo.getCssName("noteEditor"), xo.getCssName("selectedEditor"), xo.getCssName("doneEditor"), xo.getCssName("twoLevelsDownEditor")]);
    DH.removeClass(editorEl,[xo.getCssName("fixed"), xo.getCssName("hovered")]);
};
...
Simplex.textareas_dispose = function(nosave){
    ...
    if(Simplex.textPriDiv) {
	DH.removeClass(Simplex.textPriDiv,xo.getCssName("editing"));
...
Simplex.hideControls = function(){ 
	...
	DH.removeClass(Simplex.__moveHovered, [xo.getCssName("highlighted"),xo.getCssName("moveHovered")]);
...
Simplex.handleMouseOver = function(e, target, options){
    ...
    if (DH.hasClass(e.target,xo.getCssName("contentTagClickable")) || DH.hasClass(e.target,xo.getCssName("contentLink"))) {
	Simplex.__overTag = 1;
	Simplex.hideMenu();
	Simplex.hideControls();
	// xo.log("over tag");
    ...
    var isMoveHandle = (e.target.id == "move");
    var isBullet = DH.hasClass(e.target,xo.getCssName("bullet"));
    var isContent = DH.hasClass(e.target,xo.getCssName("content"));
    var isProject = DH.hasClass(e.target,xo.getCssName("project"));
    ...
    if (DH.hasClass(projectEl,xo.getCssName("parent"))) return;
	...
	DH.removeClass(Simplex.__hovered_nameEl,xo.getCssName("hovered"));
	DH.removeClass(Simplex.__hovered_contentEl,xo.getCssName("hovered"));
	
	Simplex.__hovered = projectEl;
	Simplex.__hovered_nameEl = nameEl;
	Simplex.__hovered_contentEl = contentEl;
	
	DH.addClass(contentEl,xo.getCssName("hovered"));
	DH.addClass(nameEl,xo.getCssName("hovered"));
    }
    ...
    if (isMoveHandle) {
	DH.addClass(projectEl,[xo.getCssName("highlighted"),xo.getCssName("moveHovered")]);
	Simplex.__moveHovered=projectEl;
    } else {
	if (Simplex.__moveHovered) {
	    DH.removeClass(projectEl, [xo.getCssName("highlighted"),xo.getCssName("moveHovered")]);
	    Simplex.__moveHovered=undefined;
	}
    }
...
Simplex.showMenu = function() {
    Simplex.__overMenu = 1;
    if (Simplex.__hovered)
	DH.addClass(Simplex.__hovered,xo.getCssName("highlighted"));
    DH.addClass($("expandButton"),xo.getCssName("controlsShow"));
    DH.addClass($("controlsLeft"),xo.getCssName("hovered"));
}

Simplex.hideMenu = function() {
    Simplex.__overMenu = 0;
    DH.removeClass($("expandButton"),xo.getCssName("controlsShow"));
    DH.removeClass($("controlsLeft"),xo.getCssName("hovered"));
    if (Simplex.__hovered)
	DH.removeClass(Simplex.__hovered,xo.getCssName("highlighted"));
}
...
Simplex.addNote = function(projectEl) {
    if (DH.hasClass(projectEl,xo.getCssName("noted"))) return;
    ...
    DH.addClass(projectEl,xo.getCssName("noted"));
    DH.createDom({"tag":"div","cls":xo.getCssName("spacer"),"html":"."},noteContentEl);

    store.startBatch(_CMD_UPDATE_NOTE_,[projectId]);
...
Simplex.handleEnter = function(e, item, itemtext, textarea){
	...
	if (DH.hasClass(editorEl,xo.getCssName("noteEditor"))) {
	    Simplex.edit(item["id"]);
	} else if (DH.hasClass(editorEl,xo.getCssName("nameEditor"))) {
	    xo.Event.stopEvent(e);
...
Simplex.renderItem = function(id,focus){
    ...
    xo.log('renderItem-0 id='+id);

    if(!id) return;	
    var objCurr = store.find(id);

    xo.log('renderItem-0 objCurr='+objCurr);
    ...
    var classes = xo.getCssName("project");  // consider using class name "project"
    if(objCurr) {
	if(objCurr[_collapsed_]==0) classes += " " + xo.getCssName("open");
	if(objCurr[_completed_]==1) classes += " " + xo.getCssName("done");
	if(objCurr[_noted_]==1) classes += " " + xo.getCssName("noted");
	if(!Simplex.hasChildren(objCurr)) classes += " " + xo.getCssName("task");
    }
	...
	"cn" : [{
	    "tag" : "div",
	    "class" : xo.getCssName("dropTarget")
	}, {
	    "tag" : "div",
	    "class" : xo.getCssName("highlight")
	}, {
	    "tag" : "div",
	    "class" : xo.getCssName("name"),
	    "cn" : [{
		"tag" : "a",
		"href": "#" +id,
		"class" : xo.getCssName("bullet"),
		"html" : "&bull;"
	    }, {
		"tag" : "div",
		"class" : xo.getCssName("content"),
		"html" : Simplex.toHtml(objCurr[_text_])
	    }, {
		"tag" : "span",
		"class" : xo.getCssName("parentArrow")
	    }]
	}, {
	    "tag" : "div",
	    "class" : xo.getCssName("notes"),
	    "cn" : [{
		"tag" : "div",
		"class" : xo.getCssName("content"),
		"html" : (objCurr[_noted_] ? objCurr[_note_] + '<div class="'+xo.getCssName("spacer")+'">.</div>' : "")
	    }]
	}, {
	    "tag" : "div",
	    "class" : xo.getCssName("children"),
	    "cn" : [{
		"tag" : "div",
		"class" : xo.getCssName("childrenEnd")
	    }]
	}]
    };
    ...
    if(xo.isDef(objCurr[_next_]) 
       && $(objCurr[_next_]) 
       && store.find(objCurr[_next_])[_parent_] == objCurr[_parent_]) {

    	insertAt = {"before": $(objCurr[_next_])};

    }
    
    xo.log('renderItem-1 insertAt=' + insertAt);
    xo.log(insertAt);
	...
	if(!insertAt || !insertAt["parent"]) {
	    xo.log('renderItem-1 ui_container='+Simplex.ui_container);
	    xo.log(Simplex.ui_container);
	    n = DH.createDom(nodeOptions, Simplex.ui_container);
	} else {
	    try {
		n = DH.insertBefore(Simplex.childrenEndDom(insertAt["parent"]),nodeOptions);
	    } catch(ex) {
		xo.log('error trying to insertAt["parent"]');
	    }
	}
    }

    xo.log('renderItem-1 n='+n);
    xo.log(n);
...
Simplex.toHtml = function(text) {
    ...
    html = html.replace(TAG_REGEXP,'$1<span class="'+xo.getCssName("contentTag")+'">#<span class="'+xo.getCssName("contentTagText")+'">$2</span><div title="Filter #$2" data-tag="#$2" class="'+xo.getCssName("contentTagClickable")+'"></div></span>').replace(URL_REGEXP,'<a href="$1" target="_blank" class="'+xo.getCssName("contentLink")+'">$1</a>').replace(SYMBOL_REGEXP,'$1<span class="'+xo.getCssName("contentTag")+'">$$<span class="'+xo.getCssName("contentTagText")+'">$2</span><div title="Symbol $$$2" data-symbol="$$$2" data-type="symbol" class="'+xo.getCssName("contentTagClickable")+'"></div></span>');
...
Simplex.item_getChildrenIndicator = function(el){
    ...
    var result = DQ.byClassName(el.childNodes, null,xo.getCssName("expandButton"));
    return result[0];

};

Simplex.projectDom = function(el) {
    while (el && !DH.hasClass(el,xo.getCssName("project"))) 
	el = el.parentNode;
    return el;
}

Simplex.nameDom = function(el) {
    var nodes = DQ.byClassName(el.childNodes, null,xo.getCssName("name"));
...
Simplex.textDom = function(el) {
    ...
    if (!nameEl) return;
    return DQ.byClassName(nameEl.childNodes, null,xo.getCssName("content"))[0];
};

Simplex.noteDom = function(el) {
    return DH.hasClass(el.childNodes[3],xo.getCssName("notes")) ? el.childNodes[3] : DQ.byClassName(el.childNodes, null,xo.getCssName("notes"))[0];
}

Simplex.noteContentDom = function(el) {
    return DH.hasClass(el.firstChild,xo.getCssName("content")) ? el.firstChild : DQ.byClassName(el.childNodes, null,xo.getCssName("content"))[0];
}
...
Simplex.childrenDom = function(el) {
    ...
    return DQ.byClassName(el.childNodes, null,xo.getCssName("children"))[0];
}
...
Simplex.childrenEndDom = function(el) {
    ...
    return DQ.byClassName(childrenEl.childNodes, null,xo.getCssName("childrenEnd"))[0];
}
...
Simplex.item_getNodeChildItem = function(el, index){
	...
	var i;
	var n = DQ.byClassName(el.childNodes, null,xo.getCssName("project"));
	n = DQ.byTag(n, "DIV");
...
Simplex.handleShowCompletedButton = function(e,target,options) {
    ...
    var hideEl = DQ.byClassName(buttonEl.childNodes,null,xo.getCssName("hide"))[0];
    var showEl = DQ.byClassName(buttonEl.childNodes,null,xo.getCssName("show"))[0];
    if (DH.hasClass(document.body,xo.getCssName("showCompleted"))) {
	DH.removeClass(document.body,xo.getCssName("showCompleted"));
	...
	DH.addClass(document.body,xo.getCssName("showCompleted"));
...
Simplex.hideCompleted = function() {
    return !DH.hasClass(document.body,xo.getCssName("showCompleted"));
}
...
Simplex.toggleDone = function(el) {
	...
	DH.removeClass(el,xo.getCssName("done"));
    } else {
	store.update(objCurr, _completed_, 1);
	DH.addClass(el,xo.getCssName("done"));
    }
...
Simplex.expand = function(el,nocapture) {
    ...
    DH.addClass(el,xo.getCssName("open"));
...
Simplex.collapse = function(el) {
    ...
    DH.removeClass(el,xo.getCssName("open"));
}
...
Simplex.zoomTo = function(id) {
    ...
    if (!id || id=="simplex") {
	id = "";
	DH.removeClass(Simplex.__root, xo.getCssName("parent"));
	DH.addClass(Simplex.__root, xo.getCssName("selected"));
    } else {
	DH.removeClass(Simplex.__root, xo.getCssName("selected"));
	DH.addClass(Simplex.__root, xo.getCssName("parent"));
    }
	...
	while (objCurr) {
	    var cn = $(objCurr[_id_]);
	    DH.removeClass(cn,xo.getCssName("parent"));
	    DH.removeClass(cn,xo.getCssName("selected"));
	...
	while (objParent) {
	    ...
	    DH.removeClass(pn, xo.getCssName("selected"));
	    DH.addClass(pn,xo.getCssName("parent"));
	    if (!objParent[_parent_]) break;
	    objParent = store.find(objParent[_parent_]);
	}
    }

    DH.removeClass(el,xo.getCssName("parent"));
    DH.addClass(el,xo.getCssName("selected"));
...
Simplex.append = function(objParent,objChild) {
    ...
    DH.removeClass(pn,xo.getCssName("task"));
...
Simplex.outdent = function(id,nocapture) {
	...
	DH.addClass($(objParent[_id_]),xo.getCssName("task"));
    }
...
Simplex.nonTextAreaClick = function(e,target,options) {
    ...
    var projectEl = Simplex.projectDom(e.target);
    var projectId = projectEl["id"];
    if (DH.hasClass(e.target,xo.getCssName("bullet"))) {
	xo.Event.stopEvent(e);
	Simplex.zoomTo(projectId);
    } else if (DH.hasClass(projectEl,xo.getCssName("parent"))) {
	xo.Event.stopEvent(e);
	Simplex.zoomTo(projectId);
    } else if (DH.hasClass(e.target,xo.getCssName("delete"))) {
	xo.Event.stopEvent(e);
	Simplex.hideMenu();
	Simplex.remove(projectId);
    } else if (DH.hasClass(e.target,xo.getCssName("complete"))) {
	xo.Event.stopEvent(e);
	Simplex.hideMenu();
	Simplex.toggleDone(projectEl);  // starts its own batch
    } else if (DH.hasClass(e.target,xo.getCssName("note"))) {
	xo.Event.stopEvent(e);
	Simplex.hideMenu();
	if (!DH.hasClass(projectEl,xo.getCssName("noted"))) {
	    Simplex.addNote(projectEl);  // starts its own batch
	}
	Simplex.editNote(projectEl);
    } else if (DH.hasClass(e.target,xo.getCssName("contentTagClickable"))) {
	xo.Event.stopEvent(e);
...
Simplex.ui_load = function() {
    ...
    DH.addClass(Simplex.__root,xo.getCssName("project"));
    DH.addClass(Simplex.__root,xo.getCssName("open"));
    DH.addClass(Simplex.__root,xo.getCssName("selected"));

    Simplex.ui_container = Simplex.childrenDom(Simplex.__root);

    Simplex.editorPri = DH.createDom({"tag":"div","cls":xo.getCssName("editor"),"cn":[{"tag":"textarea","spellcheck":"false"}]},$("visible"));
    Simplex.editorSec = DH.createDom({"tag":"div","cls":xo.getCssName("editor"),"cn":[{"tag":"textarea","spellcheck":"false"}]},$("visible"));
    ...
    xo.log('textPri='+Simplex.textPri);
    xo.log('textSec='+Simplex.textSec);
...
Simplex.searchCancel = function(e,target,options) {
    $("searchBox").value = "";
    DH.removeClass(document.body,xo.getCssName("searching"));
}
...
Simplex.search = function(e,target,options) {
    ...
    if (query.length) {
	DH.addClass(document.body,xo.getCssName("searching"));
    } else {
	DH.removeClass(document.body,xo.getCssName("searching"));
    }
	...
	if (Simplex.matches(el,query)) {
	    DH.addClass(el,[xo.getCssName("matches"),xo.getCssName("terminalMatch")]);
	    ...
	    DH.removeClass(el,[xo.getCssName("matches"),xo.getCssName("terminalMatch"),xo.getCssName("uncompletedDescendantMatches")]);
	    DH.removeClass(nameEl,xo.getCssName("matches"));
	}
...
Simplex.ui_completedVisible = function(){
	return (DH.hasClass(Simplex.ui_container,xo.getCssName("hidecompleted")) ? false : true);
};
//=============================================UTILITY FUNCTIONS=============================================
...
Simplex.loadTree = function(o) {
	...
	xo.log('loadTree-1: renderItem id='+objTemp[_id_]);
...
Simplex.saveButton_saveNow = function() {
    ...
    DH.removeClass(saveButtonEl,xo.getCssName("saving"));
    DH.removeClass(saveButtonEl,xo.getCssName("saved"));
    DH.addClass(saveButtonEl,xo.getCssName("saveNow"));
...
Simplex.saveButton_saving = function() {
    ...
    var saveButtonEl = $("saveButton");
    DH.removeClass(saveButtonEl,xo.getCssName("saveNow"));
    DH.removeClass(saveButtonEl,xo.getCssName("saved"));
    DH.addClass(saveButtonEl,xo.getCssName("saving"));
...
Simplex.saveButton_saved = function() {
    ...
    DH.removeClass(saveButtonEl,xo.getCssName("saveNow"));
    DH.removeClass(saveButtonEl,xo.getCssName("saving"));
    DH.addClass(saveButtonEl,xo.getCssName("saved"));
    Simplex.__saveButtonState = _saveButton_saved_;
}
...
Simplex.util_isItem = function(el) {
    if(!el) return;
    return DH.hasClass(el,xo.getCssName("project"));
}

//Checks whether an element is a "text node"
Simplex.util_isItemText = function(el) {
    if(!el)return;
    return (DH.hasClass(el,xo.getCssName("content")) && el.tagName != "TEXTAREA");
}

//Checks whether an element is a "children indicator" 
Simplex.util_isChildrenIndicator = function(el) {
    if(!el)return;
    return (DH.hasClass(el,xo.getCssName("expandPlus")));
}

//Checks whether an element is a "note"
Simplex.isNoteContentDom = function(el){
    if(!el) return;
    return (DH.hasClass(el,xo.getCssName("content")) && DH.hasClass(el.parentNode,xo.getCssName("notes")) && el.tagName != "TEXTAREA");
}

Simplex.util_isEditableNote = function(el){
	if(!el)return;
	return (DH.hasClass(el,xo.getCssName("note")) && el.tagName == "TEXTAREA");
}
...
Simplex.init = function (config){
    ...
    //DH.addClass(document.body,xo.getCssName("shared"));
    DH.addClass(document.body,xo.getCssName("showCompleted"));
    if (xo.isGecko)
	DH.addClass(document.body,xo.getCssName("mozilla"));
