...
/* Number of terms to print when writing treenode (Added by Jing Ding 09/05/03) */
int bow_term_print_num = 50;
...
static struct argp_option bow_options[] =
{
  ...
  {"print-word-num", TERM_PRINT_NUM, "NUM", 0,
   "The number of terms to print when writing treenodes"},
  ...
  {"lex-multiword", LEX_MULTIWORD_KEY, 0, 0,
   "Use a special lexer that includes digits in tokens, delimiting tokens "
   "only by non-alphanumeric characters except underscore ('_')."},
...
parse_bow_opt (int opt, char *arg, struct argp_state *state)
{
  switch (opt)
    ...
    case SCORE_PRINT_PRECISION:
      ...
      break;
    case TERM_PRINT_NUM:
      /* Set the number of terms to print (Added by Jing Ding 09/05/03) */
      bow_term_print_num = atoi(optarg);
      break;
    case SPLIT_SEED:
    ...
    case LEX_ALPHANUM_KEY:
      ...
      memcpy (bow_default_lexer_parameters, bow_alphanum_lexer_parameters,
	      sizeof (bow_lexer_parameters));
      break;
    case LEX_MULTIWORD_KEY:
      /* Use the multiword lexer (Added by Jing Ding, 09/02/2003) */
      memcpy (bow_default_lexer_parameters, bow_multiword_lexer_parameters,
	      sizeof (bow_lexer_parameters));
      break;
