...
/* Acceptable word characters. Added by Jing Ding (09/02/2003) */
int bow_isword (int character)
{
...
/* A lexer that keeps all alphabetic strings, delimited by
   ...
   `http://www.cs.cmu.edu' will result in the tokens `http', `www',
   `cs', `cmu', `edu'.
   Keep words connected by underscore. (Added by Jing Ding, 09/02/2003)
   */
const bow_lexer_parameters _bow_multiword_lexer_parameters =
{
  bow_isalpha,			/* begin words with an alphabetic char */
  bow_isword,			/* end words with any non-alphabetic char */
  YES,				/* strip non-alphas from end */
  NO,				/* don't toss words w/ non-alphas */
  0,				/* don't toss words with digits */
};
const bow_lexer_parameters *bow_multiword_lexer_parameters =
&_bow_multiword_lexer_parameters;
