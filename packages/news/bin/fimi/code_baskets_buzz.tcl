...
# BASED ON TAGS
#set cmd "psql -q -A -h $hostname -U $username -t -c \"select tags from xo.xo__sw__agg__url where buzz_p and tags is not null order by creation_date desc\" buzzdb"

set cmd "psql -q -A -h $hostname -U $username -t -c \"select translate(last_crawl_content,'\r\n',' ') from xo.xo__sw__agg__url where not buzz_p order by creation_date desc limit 5000\" buzzdb"
...
while {![eof $input_fp]} { 
	set basket {}
    foreach word [split [gets $input_fp]] {
