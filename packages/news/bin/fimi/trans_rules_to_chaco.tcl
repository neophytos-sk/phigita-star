...
proc get_edge_weight list {
    return [expr { 64-round(log(1+100000*[get_avg $list])) }]
}


###set fmt 101 ;# weights associated with edges and vertex number in the beginning of line
...
for {set u 1} {$u <= $num_vertices} {incr u} {
    ...
    foreach v $adj(${u}) {
	incr k
	###puts "$u 1 $v [get_edge_weight $edge_weight(${e})]"
	if { ${fmt} eq {} } {
	    puts -nonewline " $v"
	} elseif { ${fmt} eq {1} } {
	    set key [get_edge_key ${u} ${v}]
...
###puts "% num_vertices=$num_vertices num_edges=$num_edges k=$k (k/2 must be equal to N)"