...
while {![eof $fp]} {
    foreach {id url_sha1} [gets $fp] {

	set res [$conn exec "select title from xo.xo__sw__agg__url where url_sha1='${url_sha1}'"]
	set numTuples [pg_result $res -numTuples]
	set mydict [$res -dict]
	set doc($id) [dict get $mydict 0]
	$res -clear
	#set doc($id) http://buzz.phigita.net/admin/cache?url_sha1=$url_sha1
