...
set length [lindex $argv 4]

# BASED ON TAGS
set cmd "/opt/postgresql/bin/psql -q -A -h $hostname -U $username -t -c \"select title || '|' || translate(substr(last_crawl_content,0,${length}),'\\r\\n|',' ') from xo.xo__sw__agg__url where creation_date > current_timestamp-'${num_hours} hours'::interval order by creation_date desc\" buzzdb"
### set cmd "psql -q -A -h $hostname -U $username -t -c \"select url_sha1 || '|' || translate(title || ' ' || tags || ' ' || substr(last_crawl_content,0,${length}),'\\r\\n|',' ') from xo.xo__sw__agg__url where creation_date > current_timestamp-'${num_hours} hours'::interval and tags is not null order by creation_date desc\" buzzdb"
