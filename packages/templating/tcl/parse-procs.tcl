ad_library {
  ...
  @cvs-id $Id: parse-procs.tcl,v 1.37 2005/02/24 13:33:02 jeffd Exp $
}
...
ad_proc -private template::adp_parse { __adp_stub __args } {
  ...
  if { [llength [info procs ::ds_page_fragment_cache_enabled_p]]
    ...
    set template_list [ns_cache_eval ds_page_bits [ad_conn request] {}]
    lappend template_list $__adp_stub.$template_extension
    ns_cache_eval ds_page_bits [ad_conn request] { return $template_list }
  }
  ...
  if { $templated_p } {
    ...
    if { [llength [info procs ::ds_page_fragment_cache_enabled_p]]
         ...
         && [::ds_page_fragment_cache_enabled_p]
         && [::ds_collection_enabled_p] } {
      ns_cache set ds_page_bits "[ad_conn request]:$__adp_stub.$template_extension" "return $__adp_output"
    }
  ...
  if {[string equal $proc_name {}] || [string compare $refresh_cache "never"]} {
	...
	default {
      ...
      if {[llength [info procs ::ds_enabled_p]]} { 
        proc ::template::code::${type}::$file_stub {} "if {\[::ds_enabled_p\] && \[::ds_collection_enabled_p\]} { ds_profile start $file_stub.$type }
uplevel {
$code
}
