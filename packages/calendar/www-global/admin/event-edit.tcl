...
Page new -master ::xo::ui::DefaultMaster -title "Edit Event" -appendFromScript {
    ...
    Panel new -autoHeight true -width 465 -header true -border true -headerAsText true -style "'margin-left:auto;margin-right:auto'" -title "'Edit Event'" -appendFromScript {
...
### MIXIN RULES
	...
	MixinRule new -applyTo event_form -check ALL -guard {
	    {[string is integer -strict [::xo::kit::queryget id]] }
	} -instproc render {visitor} {
	    
	    set id [::xo::kit::queryget id]
	    set eventdata [::db::Set new \
                              -pool agendadb \
			      -type ::agenda::Event \
			      -where [list "id = [ns_dbquotevalue $id]"]]
	    $eventdata load

	    if { [$eventdata emptyset_p] } {
		rp_returnnotfound
		return
	    }

	    set o [$eventdata head]
	    $o set venue [$o set venue_id]
	    $o set tags [join [$o set tags_ia] {,}]
	    my initFromObject $o
