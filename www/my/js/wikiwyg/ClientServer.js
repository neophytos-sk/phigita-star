Wikiwyg.uri = function() {
    ...
    if (! uri.match(/phigita\.net/))
        alert("This only works on phigita.net");
    return uri;
}

proto = new Subclass('Wikiwyg.ClientServer', 'Wikiwyg');

proto.saveChanges = function(oBody) {
    var self = this;
    this.current_mode.toHtml( function(html) { self.fromHtml(html) });
    self=this.mode_objects['Wikiwyg.Wikitext.ClientServer'];
    self.fromHtml(this.div.innerHTML);
    oBody.value='\n'+self.toWikitext();
}

proto.testChanges = function(oBody) {
    var self = this;
    self.switchMode('Wikiwyg.Wikitext.ClientServer');
    oBody.value=self.current_mode.textarea.value;
}
