...
proc Location {args} {
	...
	"xlabel" {
	    ...
	    set y [expr $_draw(info,$d,yoff) - 25.0]
	}
	default {
...
proc Drawable {args} {
    ...
    # debugging info
    Dputs drawable "drawable '$use(name)' offset:$use(xoff)x$use(yoff) dimensions:$use(width)x$use(height) xrange:$xrange(0),$xrange(1) yrange:$yrange(0),$yrange(1)"
}
