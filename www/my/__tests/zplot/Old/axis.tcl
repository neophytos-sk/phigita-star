...
proc AxisTicsLabels {args} {
	...
	{"xlabelshift"     "0,0"          "offset in pts of text (x, y)"}
	{"ylabel"          ""             "empty: use range,step; else, list of form: 'y1,label:y2,label:...'"}
	...
	{"ylabelshift"     "0,0"          "offset in pts of text (x,y)"}
    ...
    # now some labels
    set scount [ArgsParseNumbers $use(xlabelshift) xshift]
	...
	-location $use(xlabellocation) -ticsize $use(xticsize) -shift $xshift(0),$xshift(1) 

    set scount [ArgsParseNumbers $use(ylabelshift) yshift]
    AssertEqual $scount 2

    TicLabels -axis y -drawable $use(drawable) \
	-range $use(ylabelrange) -step $use(ylabelstep) -label $use(ylabel) -rotate $use(ylabelrotate) \
	-font $use(font) -fontsize $use(fontsize) -fontcolor $use(fontcolor) \
	-location $use(ylabellocation) -ticsize $use(yticsize) -shift $yshift(0),$yshift(1) 
}
...
proc Axis {args} {
	...
	set min [DrawableGetRangeMin $use(drawable) $use(axis)]
	set max [DrawableGetRangeMax $use(drawable) $use(axis)]
    } else {
	...
	x {
	    ...
	    if {$use(location) == ""} {
		set y [Translate $use(drawable) y [DrawableGetRangeMin $use(drawable) y]]
	    } else {
	...
	y {
	    ...
	    if {$use(location) == ""} {
		set x [Translate $use(drawable) x [DrawableGetRangeMin $use(drawable) x]]
	    } else {
...
proc TicLabels {args} {
	...
	# find stepOp
	set stepOp "+"
	if {[drawableIsLogScale $use(drawable) $use(axis)]} {
	    set stepOp "*"
	    if {$step <= 1.0} {
		set step 2.0
	    }
	...
	for {set i [expr $min $stepOp $step]} {$i <= $max} {set i [expr $i $stepOp $step]} {
	    # puts "looping $i"
	    set intI [expr int($i)]
	    if {$i == $intI} {
	...
	x {
	    ...
	    if {$use(location) == ""} {
		set ybase  [DrawableGetRangeMin $use(drawable) y]
	    } else {
		set ybase  $use(location)
	    }
	    set tybase [Translate $use(drawable) y $ybase]
	    set y      [expr  $tybase - $use(ticsize) + $shift(1)]
		...
		set tx [expr [Translate $use(drawable) x $x] + $shift(0) + $xshift]
	...
	y {
	    # take 0 value of other axis (e.g., y0 for x axis), add offset, subtract ticsize
	    if {$use(location) == ""} {
		set xbase  [DrawableGetRangeMin $use(drawable) x]
	    } else {
		set xbase  $use(location)
	    }
	    set txbase [Translate $use(drawable) x $xbase]
	    set x      [expr $txbase - $use(ticsize) + $shift(0)]
		...
		set ty [expr [Translate $use(drawable) y $y] + $shift(1) + $yshift]
...
proc TicMarks {args} {
	...
	set base [DrawableGetRangeMin $use(drawable) ${otherAxis}]
    }
    ...
    for {set val $min} {$val <= $max} {set val [expr $val $stepOp $step]} {
