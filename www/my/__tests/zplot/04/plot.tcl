...
proc PlotVerticalIntervals {args} {
	...
	{"align"      "c"           "c - center, l - left, r - right, b - both (or box), n - none"}
	{"linecolor"  "black"       "color of the line"}
...
proc PlotPoints {args} {
    ...
    for {set r 0} {$r < [TableGetNumRows $use(table)]} {incr r} {
	set x [Translate $use(drawable) x [TableGetVal $use(table) $use(x) $r]]
	set y [Translate $use(drawable) y [TableGetVal $use(table) $use(y) $r]]
...
proc PlotLines {args} {
	...
	{"style"      "x"           "box,circle,filledcircle,x,triangle,filledtriangle,..."}
	{"size"       "2"           "overall size of marker"}
	{"fill"       "false"       "fill under the line or not"} 
	{"fillcolor"  "gray"        "fill color (if used)"}
	{"fillstyle"  "solid"       "solid, boxes, circles, ..."}
	{"fillparams" "2,4"         "any params that the fill style needs"}
	{"yfillmin"   ""            "y value to fill down to (if filling); empty means min of y-range"}
	{"legend"     ""            "add this entry to the legend"}
    }
    ArgsProcessWithDashArgs PlotPoints default args use ""
    AssertNotEqual $use(table) ""

    set x   [Translate $use(drawable) x [TableGetVal $use(table) $use(x) 0]]  
    set y   [Translate $use(drawable) y [TableGetVal $use(table) $use(y) 0]]
    set plist   "$x,$y"
    ...
    if {$use(yfillmin) == ""} {
	set use(yfillmin) [DrawableGet $use(drawable) ymin]
    }
    set ymin [Translate $use(drawable) y $use(yfillmin)]

    if {[True $use(fill)]} {
	set lastP [lindex $plist 0]
	for {set r 1} {$r < [TableGetNumRows $use(table)]} {incr r} {
	    set currP [lindex $plist [expr 2*$r]]
	    set x1 [lindex [split $lastP ","] 0]
	    set y1 [lindex [split $lastP ","] 1]
	    set x2 [lindex [split $currP ","] 0]
	    set y2 [lindex [split $currP ","] 1]
	    PsPolygon -coord "$x1,$ymin : $x1,$y1 : $x2,$y2 : $x2,$ymin" \
    ...
    # puts stderr "plist: $plist"
    PsLine -coord $plist -linecolor $use(linecolor) -linewidth $use(linewidth) 
}
