...
proc TableGetMax {args} {
    ...
    set default {
	{"table"  "default"         "table"}
	{"column" "x"               "column to get max over"}
    }
    ...
    set max $_table($use(table),$use(column),0)
    for {set r 1} {$r < $_table($use(table),rows)} {incr r} {
	set val $_table($use(table),$use(column),$r)
	if {$val > $max} {
	    set max $val
	}
    }
...
proc TableLoad {args} {
	...
	Dputs "Table: Found schema ($schema)"
	...
	Dputs "Table: Didn't find schema; using numerical column names"
