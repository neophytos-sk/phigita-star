...
EnterParagraphs.prototype.isNormalListItem = function() {

  var blocks = ["p","pre","h1","h2","h3","h4","h5","h6","code","ol","ul","li","body"];
  node = this.editor._getFirstAncestor(this.editor._getSelection(),blocks);
...
EnterParagraphs.prototype.isElementNode = function(_xo_node) {
    if (_xo_node) {
...
EnterParagraphs.prototype.isEmptyText = function(_xo_text) {
     _xo_regexp = /(\&nbsp\;|\<[^\>]*\/?\>|[ \t\n\r\s\xAD\xA0])*/g;
    return _xo_text.replace(_xo_regexp,'').length==0;
}

EnterParagraphs.prototype.isEmptyNode = function(_xo_node) {
    if (_xo_node && _xo_node.nodeType == Node.ELEMENT_NODE ) {
	if (this.isEmptyText(_xo_node.textContent)) {
	    return true;
	} else {
	    return false;
	}
...
EnterParagraphs.prototype.textToStart = function(_xo_rng) {

    sel = this.editor._getSelection();
...
EnterParagraphs.prototype.textToEnd = function(_xo_rng) {
	...
	if (this.isEmptyText(_xo_text_to_end)) {
		...
		if ( empty_next_p && this.isElementNode(_xo_fan_NS)) {
		   ...
		   _xo_stop_p = true;
		    ...
		    this.editor._doc.execCommand('formatblock',false,'p');
