...
function parseNode(node, mode, list, indent) {
	...
	var alt = Ext.decode(node.getAttribute('alt'));
...
Xinha.prototype.getHtmlFromStx = function(stx) {
	...
	if ( preformatted_p && prev_indent_level < this_indent_level ) {
	    ...
	    special_text += para[i].replace(/^\s/gm,'') + "\n\n";
	...
	pre_part = para[i].match(/::|%%|##[ \t\n\r]*$/);
	if (pre_part) {
	    //if (window['console']) { console.log('preformatted text coming up'); }
	    special_text_handler=pre_part;
	    preformatted_p=1;

	    para[i] = para[i].replace(/::|%%|##[ \t\n\r]*$/,'');
	    // HERE: we do not really have to skip the sections below, might remove li items if :: in the end of the li para
	    //continue;
	} 
	// line below used to check the else branch, i.e. when pre_part is null
	// if (window['console']) { console.log(para[i].length); }
...
//para.replace(/(^|[^\x15])\x06([^\x15\x06]\x15\x06)+([^\x15\x16])+/g,'$1<font class="$2">$3</font>');
    ...
    para = para.replace(/{(image):([0-9]+)\s*( left| center| right)?\s*}/g,'<img class="$3" src="http://my.phigita.net/media/view/$2?size=240" alt="{filetype:\'$1\',identifier:\'$2\'}" />');
    para = para.replace(/{(image):([0-9]+)\s*( left| center| right)?\s*\|\s*([^\{\}\r\n]*)}/g,'<img class="$3" src="http://my.phigita.net/media/view/$2?size=240" alt="{filetype:\'$1\',identifier:\'$2\'}" title="$4" />');
...
Xinha.prototype.ImageDialog = function() {
  ...
  return {
	...
	if ( !img ) {
	    imgHTML = '<img class="center" alt="{filetype:\'' + data.get('filetype') + '\', identifier:\'' + data.get('id') + '\'}" src="' + url + '" style="display:block;padding:15px;margin-left:auto;margin-right:auto;" />';
	    // if ( window['console'] ) { console.log(imgHTML); }
	    editor.insertAtCursor(imgHTML + (Xinha.is_gecko ? editor.cc : ''));
	    if (Xinha.is_gecko) editor.findCC();
	} else {
	    img.src = url;
	    img.setAttribute('alt','{filetype:"' + data.get('filetype') + '", identifier:"' + data.get('id') + '"}');
	    //img.setAttribute('filetype',data.get('filetype'));
	    //img.setAttribute('identifier', data.get('id'));
