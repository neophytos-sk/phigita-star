...
Ext.ux.Crypto.AES = function() {
  ...
  var addRoundKey = function(state, w, rnd, Nb) {  // xor Round Key into state S [§5.1.4]
      for (var r=0; r<4; r++) {
  ...
  return {
    ...
    decrypt : function(ciphertext, password, nBits) {
        ...
        var pwBytes = new Array(nBytes);
        for (var i=0; i<nBytes; i++) pwBytes[i] = password.charCodeAt(i) & 0xff;
        var pwKeySchedule = this.keyExpansion(pwBytes);
        var key = this.cipher(pwBytes, pwKeySchedule);
        key = key.concat(key.slice(0, nBytes-16));  // key is now 16/24/32 bytes long
