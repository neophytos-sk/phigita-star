...
Xinha.prototype.isNormalListItem = function() {
  ...
  var node = this._getFirstAncestor(this.getSelection(),blocks);
...
Xinha.prototype.textToStart = function(_xo_rng) {
    ...
    var sel = this.getSelection();
    var blocks = ["p","pre","h1","h2","h3","h4","h5","h6","code","ol","ul","li","body"];
    var _xo_node = this._getFirstAncestor(sel,blocks);

    if (this.isElementNode(_xo_node)) {
		var rng_clone = _xo_rng.duplicate();
...
Xinha.prototype.textToEnd = function(_xo_rng) {
    ...
    var sel = this.getSelection();
    var blocks = ["p","pre","h1","h2","h3","h4","h5","h6","code","ol","ul","li"];
    var _xo_node = this._getFirstAncestor(sel,blocks);
...
Xinha.prototype.onKeyPress = function(ev) {
  // Shortcuts
  if(this.isShortCut(ev)) {
    switch(this.getKey(ev).toLowerCase()) {
      case 'n':
  ...
  switch(ev.keyCode) {
    ...
    case 46: // KEY delete
		...
		{
			var _xo_p = rng.parentElement(); //returns the next outermost html tag that holds both ends
