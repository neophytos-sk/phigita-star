...
DHTMLSuite.dragDrop.prototype = {
	...
	__initDragDropScript : function()
	{
		...
		for(var no=0;no<this.dragDropSourcesArray.length;no++){
			...
			eval("this.dragDropSourcesArray[" + no + "][0].onmousedown =function(e,index){ DHTMLSuite.variableStorage.arrayOfDhtmlSuiteObjects[" + ind + "].__initDragDropElement(e," + no + "); }");	
	...
	__moveDragableElement : function(e)
	{
		var ind = this.objectIndex;		
		if(document.all)e = event;		
		if(this.dragDropTimer<5)return;	
