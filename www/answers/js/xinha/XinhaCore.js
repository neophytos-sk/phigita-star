  ...
  /*--------------------------------------:noTabs=true:tabSize=2:indentSize=2:--
    ...
    --  $LastChangedDate: 2007-02-03 02:11:56 +1300 (Sat, 03 Feb 2007) $
...
Xinha.version =
{
  ...
  'Date'      : '$LastChangedDate: 2007-02-03 02:11:56 +1300 (Sat, 03 Feb 2007) $'.replace(/^[^:]*: ([0-9-]*) ([0-9:]*) ([+0-9]*) \((.*)\) \$/, '$4 $2 $3'),
  'Revision'  : '$LastChangedRevision: 711 $'.replace(/^[^:]*: (.*) \$$/, '$1'),
...
Xinha.Config = function()
{
  ...
  this.btnList =
  {
    createlink: [ "Insert Web Link", "link-button-small.gif", false, function(e) { e._createLink(); } ],
    bold: [ "Bold", "bold-button-small.gif", false, function(e) { e.execCommand("bold"); } ],
    italic: [ "Italic", "italic-button-small.gif", false, function(e) { e.execCommand("italic"); } ],
    insertunorderedlist: [ "Bulleted List", "list-button-small.gif", false, function(e) { e.execCommand("insertunorderedlist"); } ],
    highlight: [ "Highlight Text", "color-button-small.gif", false, function(e) { e.execCommand("highlight"); } ],
...
Xinha.prototype.generate = function ()
{
  ...
  if ( textarea.form )
  {
	...
	//AK Change
		...
		alert(s);
		editor._textArea.value = editor.getStxFromHtml(editor.outwardHtml(editor.getHTML()));
  ...
  if ( !text )
  {
    ...
    if ( this.config.statusBar && !noStatus )
    {
      ...
      for ( var i = ancestors.length; --i >= 0; )
      {
		...
		// AK Change
		switch (txt) {
			case "h6" : txt = "code"; break;
			case "span" : txt = "highlight"; break;
			case "a" : txt = "link"; break;
			case "b" : txt = "bold"; break;
			case "i" : txt = "italic"; break;
			case "u" : txt = "underline"; break;
		}		
		// AK Change - end
...
//alert('highlight2');
		btn.state("active", doc.queryCommandValue("hilitecolor") != "transparent");
//alert('highlight3');
...
//alert('highlight4');
	  ...
	  case "insertorderedlist":
	  	btn.state("active", doc.queryCommandState(cmd));
...
Xinha.prototype._comboSelected = function(el, txt)
{
  ...
  switch (txt)
  {
    ...
    case "formatblock":
	  ...
	  if (value == "<h6>")
	  {
	  	//converts it first into stx and puts it as the content of <code>
		
		var blocks = ["p","pre","h1","h2","h3","h4","h5","h6","code","ol","ul","body"];
	  	var deepestAncestor = this._getFirstAncestor(this._getSelection(),blocks);
		stx = this.getStxFromHtml(deepestAncestor.innerHTML);
		deepestAncestor.innerHTML = stx;
	  }
	  //AK change -- finished
  ...
  switch (cmdID)
  {
	...
	case "highlight":
		...
		{ 
			...
			//alert(this._doc.queryCommandValue("hilitecolor"));
			if (this._doc.queryCommandValue("hilitecolor") != "transparent")
				this._doc.execCommand("removeformat",UI,param);
			else {
				this._doc.execCommand("removeformat",UI,param);
				this._doc.execCommand("hilitecolor", false, "#dee7ec");
			}
			this._doc.execCommand ('styleWithCSS',false,true);
		}
		catch (exx) { alert(exx); }
    ...
    default:
      try
      {
		...
		{
			//if is not to be cleared, clear the others so the new one will be put
			if (this._doc.queryCommandState(cmdID) == false)
			{
				this.fullwordSelection();
				this._doc.execCommand("unlink",UI,param);
				this._doc.execCommand("removeformat",UI,param);
			}
			else
			{
