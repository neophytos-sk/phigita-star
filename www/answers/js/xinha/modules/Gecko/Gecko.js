  ...
  /*--------------------------------------:noTabs=true:tabSize=2:indentSize=2:--
    ...
    --   that way.
    --
    --  $HeadURL: http://svn.xinha.python-hosting.com/trunk/modules/Gecko/Gecko.js $
    --  $LastChangedDate: 2007-01-31 02:09:44 +1300 (Wed, 31 Jan 2007) $
...
Gecko._pluginInfo = {
  name          : "Gecko",
  origin        : "Xinha Core",
  version       : "$LastChangedRevision: 707 $".replace(/^[^:]*: (.*) \$$/, '$1'),
  developer     : "The Xinha Core Developer Team",
  developer_url : "$HeadURL: http://svn.xinha.python-hosting.com/trunk/modules/Gecko/Gecko.js $".replace(/^[^:]*: (.*) \$$/, '$1'),
  sponsor       : "",
  sponsor_url   : "",
  license       : "htmlArea"
};
...
Gecko.prototype.onKeyPress = function(ev)
{
  ...
  switch(editor.getKey(ev))
  {
    ...
    {      
		//AK Change
		/*var range = editor._createRange(s);
		//alert('1');
		if (range.collapsed) //dev einai selection alla cursor
		{
			var l = editor._getFirstAncestor(s, 'a');
			if ( l )
				//alert(range.startOffset + " " + range.startContainer.length)
...
/*  This is now handled by a plugin  
	...
	case 37: //left
	{
		...
		{
			var blocks = ["a","span"];
			var l = editor._getFirstAncestor(s, blocks);
			if ( l )
	...
	//AK Change
		...
		var _xo_first_ancestor_node = this.editor._getFirstAncestor(this.editor._getSelection(),blocks);
		var _xo_regexp = /(\&nbsp\;|\<br[^\>]*\>|\s)*/g;
		
		var _xo_merge_paragraphs = false;
		var _xo_FAN=_xo_first_ancestor_node;
		var _xo_PS = _xo_FAN.previousSibling;
		var _xo_NS = _xo_FAN.nextSibling;
		...
		if (_xo_merge_paragraphs)
		{
			if (ev.keyCode == 8 && _xo_PS) //backspace
				if (_xo_PS.tagName == "P")
				{
				    ...
				    var _xo_DF = rng.extractContents();
