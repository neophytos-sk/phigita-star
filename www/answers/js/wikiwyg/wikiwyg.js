...
proto.modeDescription = 'Design Mode';
...
if (Wikiwyg.is_ie) {
	alert('save your changes when finished');
...
proto.do_spellcheck = function() {
	SC.buttonPress('wikidiv');
}
...
proto.set_design_mode_early = function(wikiwyg) {
	...
	this.get_edit_window().onload=function() {this.get_edit_document().designMode = 'on';}
}
...
proto.modeDescription = 'Wiki Mode';
...
proto.walk_pre = function(element,prefix) {
    if (!element) return;
    for (var part = element.firstChild; part; part = part.nextSibling) {
        if (part.nodeType == 1) {
            this.dispatch_formatter(part);
...
proto.format_pre = function(element) {
	...
	level = parentNode.list_type.length;
	if (!level) level=1;
    }
    var markup = this.config.markupRules['pre'];
    this.appendOutput('::\n');
    prefix= '\n' + ' '.times(2*level);
    this.walk_pre(element,prefix);
    this.chomp();
    this.insert_new_line();
}
...
proto.make_wikitext_link = function(label, href, element) {
    ...
    if (! href) {
        ...
        this.appendOutput(before + label + after + ':' + href);
    }
}
