...
Ext.ux.HtmlEditor = Ext.extend(Ext.form.Field, {
	...
	xinha_plugins = null;
	xinha_editors = xinha_editors ? xinha_editors : [ ta.dom ];
	...
	var editors = Xinha.makeEditors(xinha_editors, xinha_config, xinha_plugins);
	this.startEditors(editors);
	this._ux_editor=editors[xinha_editors[0]];
    },
    syncValue : function(){
	var _ux_e=this._ux_editor;
	var _ux_s = _ux_e.outwardHtml(_ux_e.getHTML());
	_ux_e._textArea.value = _ux_s.replace(/\xAD\xA0/g,'');
    },
	startEditors : function(editors) {
    ...
    setValue : function(v){
	Ext.ux.HtmlEditor.superclass.setValue.call(this, v);
	this.pushValue();
    },
    pushValue : function(){
    },
...
_editor_url = "http://www.phigita.net/lib/xinha/";
_editor_lang = "en";
...
Xinha.prototype.generate = function ()
{
  ...
  if ( textarea.form )
  {
      ...
      // onsubmit get the Xinha content and update original textarea.
		...
		alert('submit'+editor._textArea.value);
	        return true;
	      }
	    );
...
Xinha.prototype._outwardHtml = function(html) { return html; }
Xinha.prototype._inwardHtml = function(html) { return html; }

Xinha.prototype.outwardHtml = function(html)
{ 

  html =  this._outwardHtml(html);
...
Xinha.notImplemented = function(methodName) {
  throw new Error("Method Not Implemented", "Part of the HtmlEditor has tried to call the " + methodName + " method which has not been implemented.");
}
*/
...
Xinha.prototype.inheritStyle = function () {
    ...
    _xo_el.set({_xo:'1'});
    for (var i=0;i<_xo_styleClasses.length;i++) {
...
Xinha.prototype._xo_garbageCollect = function (_xo_node) {
  if (this._editMode == 'wysiwyg') {
	...
	rng = this.createRange(sel);
        ...
        for (var i=0;i<this._aux.length;i++) {
...
Xinha.prototype.onKeyPress = function(ev)
{
  ...
  {    
	...
	_xo_fan = editor._getFirstAncestor(editor.getSelection(),_xo_blocks);
...
Xinha.prototype.isNormalListItem = function() {
  ...
  node = editor._getFirstAncestor(editor.getSelection(),blocks);
...
Xinha.prototype.onKeyPress = function(ev)
{
  ...
  if (ev.keyCode == 13 && !ev.shiftKey && editor._iframe.contentWindow.getSelection)
  {
  ...
  {
  	var blocks = ["p","body"];
	var _xo_first_ancestor_node = editor._getFirstAncestor(editor.getSelection(),blocks);
...
Xinha.prototype.textToStart = function(_xo_rng) {
    ...
    _xo_node = editor._getFirstAncestor(sel,blocks);
	...
	if (this.isEmptyText(_xo_text_to_end)) {
		...
		if ( empty_next_p && this.isElementNode(_xo_fan_NS)) {
		   ...
		   _xo_stop_p = true;
		    ...
		    editor._doc.execCommand('formatblock',false,'p');
...
//		alert('para' + i + ':' + para[i] + ' sct:' + stack_close_tags + ' sil:' + stack_indent_levels + ' level: ' + this_indent_level);
            for (var j=stack_indent_levels.length-1; j>=0; j--) {
...
//					alert('result:' + result);
    ...
    if ( prev_preformatted_p ) {
	//	result += [Special_Text_Handler ${special_text_handler} ${edit_p} ${special_text}]
	    prev_preformatted_p = 0;
    }

    for (var j=0;j<stack_close_tags.length;j++) {
...
Xinha.prototype.getHtmlForStxSymbol = function(para,defaultString,regexstring,newString) {
    objRegExp = new RegExp(regexstring, "g");
...
//		   prependText:'Sort by... ',
	...
	paramNames:{'start':'x_offset','limit':'x_limit'},
	store:ds,
