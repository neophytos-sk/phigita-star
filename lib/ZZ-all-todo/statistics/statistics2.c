...
{
    Need_Float(p);
    rb_float_new(bindens(FIX2INT(n), RFLOAT(p)->value, FIX2INT(x)));
}
...
{
    Need_Float(p);
    rb_float_new(bindist(FIX2INT(n), RFLOAT(p)->value, FIX2INT(x)));
}
...
{
    Need_Float(p);
    rb_float_new(bindist(FIX2INT(n), 1.0 - (RFLOAT(p)->value), FIX2INT(n) - FIX2INT(x)));
}
...
{
    Need_Float(m);
    rb_float_new(poissondens(RFLOAT(m)->value, FIX2INT(x)));
}
...
{
    Need_Float(m);
    rb_float_new(poissondist(RFLOAT(m)->value, FIX2INT(x)));
}
...
{
    Need_Float(m);
    rb_float_new(1.0 - poissondist((RFLOAT(m)->value), FIX2INT(x) - 1));
}
