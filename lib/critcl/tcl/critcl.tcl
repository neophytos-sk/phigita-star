...
namespace eval ::critcl {
  ...
  proc ininame {ininame} {
      set v::ininame $ininame
  }
  ...
  proc ldflags {args} {
    ...
    proc cinit {text exts {file ""}} {
    set digest [md5_hex $text]
	if { $file eq {} } {
	    set file [file_normalize [info script]]
	}

	#ns_log notice cinit file=$file
...
"]
    ...
    proc cbuild {{file ""} {load 1} {prefix {}} {silent ""} {pretend_load 0} {base ""}} {
	...
	set ininame [string totitle [string map {"-" "_"} $ininame]]
...
# line 1 \"MyInitTkStubs\"
  ...
  if (!MyInitTkStubs(ip)) return TCL_ERROR;
	...
	lappend copts -x $v::options(language)
      ...
      if {$v::options(I) != ""} {
	  #lappend copts $c::include
	  foreach includedir $v:::options(I) {
	      lappend copts "$c::include$includedir"
	  }
      }
	...
	if {[string index $x 0] == "-"} {
      ...
      if {($pretend_load || $load || $link) && !$v::failed} {
        set cmdline $c::link
...
#       append cmdline " bufferoverflowU.lib";# msvc >=1400 && <1500 for amd64
	...
	# ns_log notice "ns_moduleload $ininame $target"
	# ns_moduleload $ininame $target
        load $target $ininame
    }
