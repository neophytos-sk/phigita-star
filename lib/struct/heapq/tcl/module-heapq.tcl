package provide struct/heapq 0.1
...
::critcl::cinit {
    // init_text
    cbt_InitModule();
    Tcl_CreateObjCommand(ip, "::struct::heapq::create", heapq_CreateCmd, NULL, NULL);
    Tcl_CreateObjCommand(ip, "::struct::heapq::push", heapq_PushCmd, NULL, NULL);
    Tcl_CreateObjCommand(ip, "::struct::heapq::pop", heapq_PopCmd, NULL, NULL);
    Tcl_CreateObjCommand(ip, "::struct::heapq::top", heapq_TopCmd, NULL, NULL);
    Tcl_CreateObjCommand(ip, "::struct::heapq::isempty", heapq_IsEmptyCmd, NULL, NULL);
    Tcl_CreateObjCommand(ip, "::struct::heapq::size", heapq_SizeCmd, NULL, NULL);
    Tcl_CreateObjCommand(ip, "::struct::heapq::destroy", heapq_DestroyCmd, NULL, NULL);
...
critcl::ccode {
    ...
    #define HEAPQ_CMD(s,internal)      sprintf((s), "_HEAP%p", (internal))
	...
	*/
    ...
    static int 
    heapq_ReturnHandle (Tcl_Interp  *interp, cbt_InternalType *internal, int setVariable, Tcl_Obj *varNameObj) {
	char        handle[80], *varName = NULL;
	//Tcl_CmdInfo cmdInfo;

	DBG(fprintf(stderr,"heapq_ReturnHandle\n"));
	...
	HEAPQ_CMD(handle,internal);
	
	DBG(fprintf(stderr,"heapq_ReturnHandle: create handle => %s\n",handle));

	// TSD(dontCreateObjCommands)
	if (setVariable) {
	    varName = Tcl_GetString(varNameObj);
	    Tcl_SetVar(interp, varName, handle, 0);
	    DBG(fprintf(stderr,"heapq_ReturnHandle: set %s %s\n",varName,handle));
	}
    ...
    static int 
    heapq_CreateCmd (ClientData  clientData, Tcl_Interp *interp, int objc, Tcl_Obj * const objv[] ) {
	...
	heapq_InternalType *internal = heapq_AllocInternal(interp);
	if (internal == NULL) {
	    return TCL_ERROR;
	}
	...
	return heapq_ReturnHandle(interp,  internal, setVariable, newObjName);
    }
