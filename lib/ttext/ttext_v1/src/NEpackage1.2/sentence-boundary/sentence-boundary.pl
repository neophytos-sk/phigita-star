...
if (defined ($opt_h) || !defined ($opt_i) || !defined ($opt_d) || !defined ($opt_o))
{
   $opt_h = 1;
   print "Usage: boundary.pl -d honorifics_file -i input_file -o output_file\n";
   exit;
}
...
sub Capital
{
   ...
   # HERE ND: Added Greek capital letters.
...
sub Boundary
{
   ...
   # HERE: Debug
   #print "prediction=$prediction, candidate=$candidate, wm2=$wm2, wm1=$wm1, prefix=$prefix, suffix=$suffix, wp1=$wp1, wp2=$wp2, wm2C=$wm2C, wm1C=$wm1C, prC=$prC, suC=$suC, wp1C=$wp1C, wp2C=$wp2C\n";
