			...
			if($checksum == 0)
			  {
				...
				elsif( exists $universityHash{$uCaseString} && ($compareString =~/^[A-Z]/))
				  {
 					$tagclass = 400;
				  }
				elsif(exists $festdayHash{$uCaseString})
				  {
					$tagclass = 220;
				  }
				elsif( exists $artworkHash{$uCaseString})
				  {
					$tagclass = 690;
				  }
				elsif( exists $bookHash{$uCaseString})
				  {
					$tagclass = 610;
				  }
				elsif( exists $corporationHash{$uCaseString} && ($compareString =~/^[A-Z]/))
				  {
					$tagclass = 370;
				  }
				elsif( exists $foodHash{$uCaseString})
				  {
					$tagclass = 660;
				  }
				elsif( exists $buildingHash{$uCaseString} && ($compareString =~/^[A-Z]/))
				  {
					$tagclass = 840;
				  }
				elsif( exists $journalHash{$uCaseString})
				  {
					$tagclass = 620;
				  }
				elsif( exists $medicalHash{$uCaseString})
				  {
					$tagclass = 640;
				  }
				elsif( exists $peopleHash{$uCaseString} && ($compareString =~/^[A-Z]/))
				  {
					$tagclass = 320;
				  }
		...
		for($i = 0; $i <= ($#orgWordlist-6); $i++)
		  {
			...
			if($checksum == 0)
			  {		
				...
				elsif( ($head =~ m/([Ss]outh)|([Nn]orth)|([Ww]est)|([Ee]ast)/) && (exists $riverHash{uc($tailString)})&& ($tailString =~/^[A-Z]/))
				  {
					$tagclass = 870; 
				  }
				elsif( ($head =~ m/([Ss]outh)|([Nn]orth)|([Ww]est)|([Ee]ast)/) && (exists $mountHash{uc($tailString)})&& ($tailString =~/^[A-Z]/))
				  {
					$tagclass = 850; 
				  }
				elsif( exists $universityHash{$uCaseString})
				  {
					$tagclass = 400;
				  }
				elsif(exists $festdayHash{$uCaseString})
				...
				elsif( exists $creatgroupHash{$uCaseString} && ($compareString =~/^[A-Z]/))
				  {
					$tagclass = 380;
				  }
				elsif( exists $performHash{$uCaseString} && ($compareString =~/^[A-Z]/))
				  {
					$tagclass = 240;
				  }
				elsif( exists $medicalHash{$uCaseString})
				...
				elsif( exists $countryHash{$uCaseString} && ($compareString =~/^[A-Z]/))
				  {
					$tagclass = 830;
				  }
			...
			if($checksum == 0)
			  {
				...
				elsif( ($head =~ m/([Ss]outh)|([Nn]orth)|([Ww]est)|([Ee]ast)/) && (exists $cityHash{uc($tailString)})&& ($tailString =~/^[A-Z]/))
				  {
					$tagclass = 820; 
				  }
				elsif( exists $universityHash{$uCaseString})
				...
				elsif( exists $riverHash{ $uCaseString} && ($compareString =~/^[A-Z]/))
				  {
					$tagclass = 870;
				  }
				elsif( exists $mountHash{ $uCaseString} && ($compareString =~/^[A-Z]/))
				  {
					$tagclass = 850;
				  }
				elsif( exists $medicalHash{$uCaseString})
				...
				elsif( exists $locHash{$uCaseString} && ($compareString =~/^[A-Z]/))
				  {
					$tagclass = 8000;
				  }
			...
			if($checksum == 0)
			  {
				...
				elsif( exists $plantHash{$uCaseString})
				  {
					$tagclass = 360;
				  }
				elsif( exists $universityHash{$uCaseString})
				...
				elsif( exists $artworkHash{$uCaseString} && ($compareString =~/^[A-Z]/))
				  {
					$tagclass = 690;
				  }
				elsif( exists $bookHash{$uCaseString})
				...
				elsif( exists $cityHash{$uCaseString} && ($compareString =~/^[A-Z]/))
				  {
					$tagclass = 820;
				  }
				elsif( exists $foodHash{$uCaseString})
				...
				elsif(exists $eventHash{$uCaseString})
				  {
					$tagclass = 230;
				  }
				elsif( exists $performHash{$uCaseString} && ($compareString =~/^[A-Z]/))
			...
			if($checksum == 0)
			  {
				...
				elsif(($uCaseString =~ m/^(\d{1,2})\s:\s(\d{2,2})\s[PA]\.M\.?/))
				  {
					$tagclass = 950;
				  }
				elsif(($uCaseString =~ m/^(\d{1,2})\s:\s(\d{2,2})\sMINUTES?/))
				  {
					$tagclass = 900;
				  }
				elsif(($uCaseString =~ m/^(\d{1,2})\s:\s(\d{2,2})\sSEC/))
				  {
					$tagclass = 900;
				  }
				elsif(($uCaseString =~ m/^(\d{1,2})\s:\s(\d{2,2})\sHOURS?/))
				  {
					$tagclass = 900;
				  }
				
				elsif( $uCaseString =~ m/^\d+\.?\d*\s(.*)/)
				  {
					if(exists $unitHash{$1}){
					  $tagclass = 910;
					}
				  }
				elsif( $uCaseString =~ m/^\d+\.?\d*(.*)/)
				  {
					if (exists $unitHash{$1}){
					  $tagclass = 910;
					}
				...
				#check to see if it is in the list
				elsif( ($head =~ m/([Ss]outh)|([Nn]orth)|([Ww]est)|([Ee]ast)/) && (exists $countryHash{uc($tailString)}) && ($tailString =~/^[A-Z]/))
				  {
					$tagclass = 830; 
				  }
				elsif( ($head =~ m/([Ss]outh)|([Nn]orth)|([Ww]est)|([Ee]ast)/) && (exists $riverHash{uc($tailString)}) && ($tailString =~/^[A-Z]/))
				...
				elsif(exists $profHash{$uCaseString})
				  {
					$tagclass =460;
				  }
				elsif( exists $universityHash{$uCaseString})
				...
				elsif( exists $bookHash{$uCaseString} && ($compareString =~/^[A-Z]/))
				  {
					$tagclass = 610;
				  }
				elsif( exists $cityHash{$uCaseString} && ($compareString =~/^[A-Z]/))
				...
				elsif(exists $teamHash{$uCaseString} && ($compareString =~/^[A-Z]/))
				  {
					$tagclass = 410;
				  }
				elsif(exists $stateHash{$uCaseString} && ($compareString =~/^[A-Z]/))
				  {
					if((length($compareString)>2) || ($compareString eq $uCaseString)) { $tagclass = 880;}
				  }
				elsif( exists $performHash{$uCaseString} && ($compareString =~/^[A-Z]/))
				  {
					$tagclass = 240;
				...
				elsif( exists $animalHash{$uCaseString})
				  {
					$tagclass = 310;
				  }
				elsif( exists $currencyHash{$uCaseString})
				  {
					$tagclass = 1000;
				  }
				elsif( exists $countryHash{$uCaseString} && ($compareString =~/^[A-Z]/))
			...
			if($checksum == 0)
			  {
				...
				elsif(($uCaseString =~ m/^(\d{1,2})\s:\s(\d{2,2})/))
				  {
					$tagclass = 900;
				  }
				elsif( $uCaseString =~ m/^\d+\.?\d*\s(.*)/)
				...
				elsif(exists $sportHash{$uCaseString})
				  {
					$tagclass =630;
				  }
				#check to see if it is in the list
				...
				elsif(exists $unitHash{$uCaseString})
				  {
					$tagclass = 910;
				  }
				elsif(exists $festdayHash{$uCaseString})
				...
				elsif (exists $religionHash{$uCaseString})
				  {
					$tagclass = 10;
				  }
				elsif(exists $langHash{$uCaseString})
				  {
					$tagclass = 270;
				  }
				elsif( exists $performHash{$uCaseString} && ($compareString =~/^[A-Z]/))
				...
				elsif( exists $polBodyHash{$uCaseString} && ($compareString =~/^[A-Z]/))
				  {
					$tagclass = 420;
				  }
				elsif( exists $medicalHash{$uCaseString})
			...
			if($checksum == 0)
			  {
				...
				elsif( $uCaseString =~ m:^\d+\S*\sYEARS?:)
				  {
					$tagclass = 900;
				  }
				elsif( $uCaseString =~ m:^(\d+\.?/*\d*)\sWEEKS?:)
				  {
					$tagclass = 900;
				  }
				elsif( $uCaseString =~ m:^(\d+\.?/*\d*)\sMONTHS?:)
				  {
					$tagclass = 900;
				  }
				elsif( $uCaseString =~ m:^(\d+\.?/*\d*)\sDAYS?:)
				  {
					$tagclass = 900;
				  }
				elsif( $uCaseString =~ m:^(\d+\.?/*\d*)\sHOURS?:)
				  {
					$tagclass = 900;
				  }
				elsif( $uCaseString =~ m:^(\d+\.?/*\d*)\sMINUTES?:)
				  {
					$tagclass = 900;
				  }
				elsif( $uCaseString =~ m:^(\d+\.?/*\d*)\sSECONDS?:)
				  {
					$tagclass = 900;
				  }
				elsif( $uCaseString =~ m:^(\d+\.?/*\d*)\sMIN\.?:)
				  {
					$tagclass = 900;
				  }
				elsif( $uCaseString =~ m:^(\d+\.?/*\d*)\sSEC\.?:)
				  {
					$tagclass = 900;
				  }
				elsif( $uCaseString =~ m:^(\d+\.?/*\d*)\sHR:)
				  {
					$tagclass = 900;
				  }
				elsif( $uCaseString =~ m:^(\d+\.?/*\d*)\sUNIT:)
				  {
					$tagclass = 900;
				  }
				elsif( $uCaseString =~ m:^(\d+\.?/*\d*)\sPERCENT:)
				  {
					$tagclass = 1020;
				  }
				elsif( $uCaseString =~ m:^(\d+\.?/*\d*)\sDOLLARS?:i)
				  {
					$tagclass = 1000;
				  }
				elsif( $uCaseString =~ m:^(\d+\.?/*\d*)\sPOUNDS?:i)
				  {
					$tagclass = 1000;
				  }
				elsif( $uCaseString =~ m:^(\d+\.?/*\d*)\sYENS?:i)
				  {
					$tagclass = 1000;
				  }
				elsif( $uCaseString =~ m:^(\d+\.?/*\d*)\sPERCENTAGES?:)
				  {
					$tagclass = 1020;
				  }
				elsif( $uCaseString =~ m/^(\(\d{3,3}\)\s\d{3,3}-\d{4,4})$/)
				  {
					$tagclass = 930;
				  }
				elsif( $uCaseString =~ m/^DOLLARS\s(\d+.*)/i)
				  {
					$tagclass = 1000
				  } 
				elsif( $uCaseString =~ m/^([A-Za-z]+DOLLARS)\s(\d+.*)/i)
				  {
					$tagclass = 1000;
				  }
				elsif( $uCaseString =~ m/^POUNDS\s(\d+.*)/i)
				  {
					$tagclass = 1000;
				  }
				elsif( $uCaseString =~ m/^NZDOLLARS\s(\d+.*)/i)
				  {
					$tagclass = 1000;
				  }
				elsif( $uCaseString =~ m/^YEN\s(\d+.*)/i)
				  {
					$tagclass = 1000;
				  } 
				elsif( $uCaseString =~ m/^(\d{1,2})\s([PA]\.M\.?)/i)
				  {
					$tagclass = 950;
				  }
				elsif( $uCaseString =~ m/^\d+\.?\d*\s(.*)/)
				...
				elsif( exists $polBodyHash{$uCaseString})
				  {
					$tagclass = 420;
				  }
				elsif( exists $medicalHash{$uCaseString})
			...
			if($checksum == 0)
			  {
				...
				elsif( $uCaseString =~ m/([Ss]outh)|([Nn]orth)|([Ww]est)|([Ee]ast)/ )
				  {
					$tagclass =8000; 
				  }
				elsif($uCaseString =~ m/^(\d{5,5})$/)
				  {
					$tagclass = 920;
				  }
				elsif(($uCaseString =~ m/(^[A-Z]\w*)-(.*)/) && (exists $corporationHash{$1} || exists $corporationHash{$2}))
				  {
					$tagclass =370;
				  } 
				elsif($uCaseString =~ m/^(\d{5,5}-\d{4,4})$/)
				  {
					$tagclass = 920;
				  }
				elsif( $uCaseString =~ m/(\$)/)
				...
				elsif( $uCaseString =~ m/^([A-Z]+[a-z]*\d+)/)
				  {
					$tagclass = 1000;
				  }  
				elsif( $uCaseString =~ m/^\d+.*%$/)
				  {
					$tagclass = 1020;
				  }  
				...
				elsif( ($uCaseString =~ m/^\d+\.?\d*(.*)/) && (exists $unitHash{$1}))
				  {
					  $tagclass = 910;
				  }
 
				#check to see if it is in the list
				elsif( exists $dayHash{$uCaseString})
				  {
					$tagclass = 960;
				  }
				elsif( exists $plantHash{$uCaseString})
				...
				elsif(exists $colorHash{$uCaseString})
				  {
					$tagclass =600;
				  }
				elsif(exists $profHash{$uCaseString})
				...
				elsif(exists $numberHash{$uCaseString})
				  {
					$tagclass =900;
				  }
				elsif( exists $universityHash{$uCaseString})
				...
				elsif(exists $dateHash{$uCaseString})
				  {
					#print"DATE -5- -------------\n";
					#print"$uCaseString\n";
					$tagclass = 960;
