...
#take first guess
res=`cat ../langclass/ShortTexts/$1.txt | $testtextcat ../langclass/fpdb.conf ../langclass/LM/ | sed -e "s/].*/]/" | sed -e "s/zh-CN/zh-Hans/" | sed -e "s/zh-TW/zh-Hant/" | sed -e "s/--utf8//" | sed -e "s/-utf8//" | sed -e "s/^\[sh\]$/\[sr-Latn\]/"`
if [ $res == "[$1]" ]; then
    exit 0
else
    exit 1
